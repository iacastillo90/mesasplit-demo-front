// src/features/CorporateView/services/corporateService.js — servicio del Super Admin Corporativo (super-admin-corporate)
// Servicio de datos corporativos: provee información resumida de ventas y salud de sucursales.
...
export async function fetchFranchiseOverview() {
  ...
  return {
    branches: [
      { id: 'b-lc', name: 'Salón Las Condes', salesTotal: 620000, activeTables: (tables ?? []).filter((t) => t.status === 'occupied').length, totalTables: 8, activeStaff: 5, avgTicket: 34500, healthStatus: 'optimal' },
