...
export default function TicketCard({ ticket, onComplete, onTogglePrepared }) {
  ...
  return (
    // Contenedor principal de la tarjeta: usa borde superior acentuado, bg-brand-800 y resplandor de alta visibilidad.
    <article
      data-has-allergy={hasAllergies ? 'true' : 'false'}
      className={`flex flex-col gap-3 rounded-2xl bg-brand-800 p-4 transition-all border-t-4 ${
        hasAllergies
          ? 'border-2 border-semantic-danger border-t-rose-500 shadow-[0_0_25px_rgba(239,68,68,0.35)]'
          : 'border border-brand-800/80 border-t-sky-500 shadow-2xl hover:border-sky-500/50'
      }`}
    >
