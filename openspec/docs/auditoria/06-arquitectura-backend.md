...
**Versión**: 1.0  
**Stack**: Java 21 LTS + Spring Boot 3.3.x  
**Paquete base**: `cl.labtab.api`  
**Fuente de verdad del esquema**: `openspec/docs/Diagrama_V3.mmd` (21 entidades)
...
| Spring Boot | 3.3.x | Framework principal — auto-configuration, embedded Tomcat |
| Spring Data JPA | 3.3.x (incluido en Boot) | Capa de persistencia — repositorios, queries JPQL |
| Hibernate | 6.x (incluido en JPA) | ORM — traducción Java ↔ PostgreSQL |
| Hypersistence Utils | 3.7.x | Tipos PostgreSQL nativos: `TEXT[]`, `JSONB` (ver Doc 06b) |
| MapStruct | 1.5.x | Mapeo Model ↔ DTO en tiempo de compilación — sin reflexión en runtime |
| JJWT | 0.12.x | Firma y validación de JWT (HS256) |
| Spring WebSocket + STOMP | 3.3.x (incluido en Boot) | Canal de tiempo real — 11 eventos del sistema |
| Springdoc OpenAPI | 2.x | Swagger UI autogenerado desde las anotaciones |
...
| JUnit 5 + Testcontainers | 1.19.x | Testing de integración contra PostgreSQL real (ver Doc 08) |
| Maven | 3.9.x | Build tool — `pom.xml` como fuente de verdad de dependencias |
