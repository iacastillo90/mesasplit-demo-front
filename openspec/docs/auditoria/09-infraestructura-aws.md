...
```dockerfile
# Dockerfile — build multi-stage: Maven compile → JRE 21 Alpine runtime
...
# ─────────────────────────────────────────────────────────
# STAGE 1: BUILD — Maven + JDK 21 compila el JAR
# ─────────────────────────────────────────────────────────
FROM eclipse-temurin:21-jdk-alpine AS builder

WORKDIR /app

# Copiar pom.xml primero — si el código cambia pero las deps no,
# Docker reutiliza la capa del mvn dependency:go-offline (caché)
COPY pom.xml .
